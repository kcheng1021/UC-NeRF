...
def main(unused_argv):
    ...
    with logging_redirect_tqdm():
        ...
        for step in tbar:
            ...
            #sky segment
            if config.model_sky:
                losses['sky_segments'] = config.sky_weight * train_utils.sky_loss(batch, renderings)

            if config.brightness_correction:
                losses['identity'] = config.idt_weight * train_utils.transformIdentityLoss(renderings)
            ...
            if config.train_render_every > 0 and step % config.train_render_every == 0:
                ...
                try:
                    test_batch, eval_camidx = next(test_dataiter)
                except StopIteration:
                    test_dataiter = iter(test_dataloader)
                    test_batch, eval_camidx = next(test_dataiter)
                test_batch = accelerate.utils.send_to_device(test_batch, accelerator.device)
                eval_camidx = accelerate.utils.send_to_device(eval_camidx, accelerator.device)

                # render a single image with all distributed processes
                rendering = models.render_image(model, accelerator,
                                                test_batch, False,
                                                train_frac, config, eval_camidx=((eval_camidx//3)*21+eval_camidx%3))
