...
def img_warping(ref_pose, src_pose, virtual_pose_ref_depth, virtual_intrinsic):
    ref_depth = virtual_pose_ref_depth
    ref_pose = ref_pose
    src_pose = src_pose
    intrinsic = torch.from_numpy(virtual_intrinsic).to(torch.float32)

    ref_depth = torch.from_numpy(ref_depth).to(torch.float32)
    ref_pose = torch.from_numpy(ref_pose).to(torch.float32)
    src_pose = torch.from_numpy(src_pose).to(torch.float32)
    mask = ref_depth > 0
    ...
    depth_tgt_insrccoord = pts_in_3D_tgt[2, :].view(ht, wd)
    ...
    valid_pixel_mask = np.logical_and(pts_in_tgt[:, :, 0] >= 0, pts_in_tgt[:, :, 1] >= 0)
    valid_pixel_mask = np.logical_and(valid_pixel_mask, pts_in_tgt[:, :, 0] < (wd - 0.5))
    valid_pixel_mask = np.logical_and(valid_pixel_mask, pts_in_tgt[:, :, 1] < (ht - 0.5))
    mask = np.logical_and(mask, valid_pixel_mask)

    return pts_in_tgt, mask


def img_warping_for_depth(ref_pose, src_pose, virtual_pose_ref_depth, virtual_intrinsic):
    ref_depth = virtual_pose_ref_depth
    ref_pose = ref_pose
    src_pose = src_pose
    intrinsic = torch.from_numpy(virtual_intrinsic).to(torch.float32)
    ...
    depth_tgt = torch.zeros_like(depth_tgt_insrccoord)
    xy = pts_in_tgt[mask].to(torch.long)
    depth_tgt[xy[:, 1], xy[:, 0]] = depth_tgt_insrccoord[mask]
...
def sky_loss(batch, renderings):
    loss = 0
    for rendering in renderings:
        weights_sum = torch.sum(rendering["weights"], dim=-1)
        sky_segment = 1 - batch['sky_segs']
        loss += torch.nn.functional.binary_cross_entropy(
            weights_sum.clip(1e-3, 1.0 - 1e-3), sky_segment)
        
    return loss

def transformIdentityLoss(renderings):
    transformation = renderings[0]['affine_trans']
    sky_transformation = None
    if "affine_trans_sky" in renderings[0].keys():
        sky_transformation = renderings[0]["affine_trans_sky"]
    Identity_matrix = torch.from_numpy(np.eye(4))[:3, :].unsqueeze(0).repeat(transformation.shape[0], 1, 1).to(transformation.device)
    loss = torch.abs(Identity_matrix - transformation)
    if sky_transformation is not None:
        loss += torch.abs(Identity_matrix - sky_transformation)
